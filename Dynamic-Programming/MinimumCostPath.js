...
const minCostPath = (matrix) => {
  ...
  for (var i = 0; i < moves.length; i++) moves[i] = new Array(m)

  // base conditions
  moves[0][0] = 0 // to reach cell (0, 0) from (0, 0) is of no moves
  for (var i = 1; i < m; i++) moves[0][i] = moves[0][i - 1] + matrix[0][i]
  for (var i = 1; i < n; i++) moves[i][0] = moves[i - 1][0] + matrix[i][0]
